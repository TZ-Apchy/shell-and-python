...
# del解除a与[1,2,3]的绑定关系(del语句用于删除对象的引用、删除列表中的元素、删除字典中的键值对、删除类的属性等)
# a = [1,2,3]
...
# print(type(a))
# b=[1,2,3,2,4,3]
# print(set(b)) # 集合是一种无序的、不包含重复元素的数据结构


# print(pow(2,3))
# import math
# print(math.pow(2,3))
# print(pow(2,3,5),8%5) # 取模运算，表示2的3次幂再对5取模(取余)
# # help(abs)
# # help('sys')
# print("hello");x = 2;print(x)

# # and(一假则假，两真才真)中如果两条语句都为真时返回的是后面那个真的值，or(一真则真，两假才假)中如果两条语句都为假时返回的是后面那个假的值
...
# # 在可变数据类型和不可变数据类型中a=a+1和a+=1的区别：
# a=[1,2,3] # 可变数据类型
# b=[1,2,3] # 可变数据类型
# print(id(a),id(b))
# a=a+[4] # 对于可变数据类型，a=a+[4]会重新开辟一块内存空间，导致a的内存地址发生改变
# b+=[4] # 对于可变数据类型，b+=[4]并不会改变b原来内存地址的指向，只是改变了它的值
# print(a,id(a),b,id(b)) # 对于可变数据类型，a和b的内存地址是不同的
# c=(1,2,3) # 不可变数据类型
# d=(1,2,3) # 不可变数据类型
# print(id(c),id(d)) # 对于不可变数据类型，c和d这里指向了同一块内存地址
# c=c+(4,) # 对于不可变数据类型，c=c+(4,)会重新开辟一块内存空间，导致c的内存地址发生改变
# d+=(4,) # 对于不可变数据类型，d+=(4,)也会重新开辟一块内存空间，导致d的内存地址也会随之改变
# print(c,id(c),d,id(d))

# # 等号赋值和copy的区别：
# a=3
# c=[1,2]
# b=a # 对于不可变数据类型，这里使得b指向了a所在的内存地址
# d=c # 对于可变数据类型，这里使得d指向了c所在的内存地址，等号赋值可变数据类型和不可变数据类型上没啥区别
# e=c.copy() # copy()会重新开辟一块内存空间，即e的内存地址指向和c、d不同
# print(a,b,c,d,e)
...
# # # exec和eval，都属于Python的内置函数，eval()和exec()函数的功能是相似的，都
# # 可以执行一个字符串形式的Python代码(代码以字符串的形式提供)，相当于一个Python的解释器；
...
# g={'a':6, 'b':8}
# t={'b':100, 'c':10}
# # g(全局命名空间globals)表示全局命名空间，t表示局部命名空间，当
# # 它和g中有重复或冲突时，以t(局部命名空间locals)的为准
